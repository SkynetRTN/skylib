...
from numpy import (
    arctan2, argsort, array, asarray, bincount, cos, dot, empty, float32, full,
...
def get_transform_stars(src_stars: Union[TList[Tuple[float, float]],
    ...
    if nref == 1 or not enable_rot and not enable_scale:
        # Pure shift
        return None, array([src_y[0] - dst_y[0], src_x[0] - dst_x[0]])
    ...
    if not enable_rot:
        ...
        return array([[py[0], 0], [0, px[0]]]), array([py[1], px[1]])
    ...
    if not enable_skew:
        ...
        if a:
            ...
            d = 0
        return array([[a, b], [c, d]]), array([dy, dx])
    ...
    return array([py[:2], px[:2]]), array([py[2], px[2]])
...
def get_transform_wcs(src_wcs: WCS, dst_wcs: WCS,
    ...
    return get_transform_stars(
        transpose([src_x, src_y]), transpose([dst_x, dst_y]),
        enable_rot=enable_rot, enable_scale=enable_scale,
        enable_skew=enable_skew)


def get_transform_features(img: Union[ndarray, ma.MaskedArray],
                           ...
                           **kwargs) -> Tuple[Optional[ndarray], ndarray]:
    """
...
def get_transform_pixel(img: Union[ndarray, ma.MaskedArray],
                        ref_img: Union[ndarray, ma.MaskedArray],
                        enable_rot: bool = True,
                        enable_scale: bool = True,
                        enable_skew: bool = True,
    ...
    if enable_rot and enable_scale and not enable_skew:
        ...
        k = dot(inv(A), b)
        return array([[k[0] + 1, k[1]], [-k[1], k[0] + 1]]), \
            array([k[2], k[3]])
    ...
    if enable_rot and not enable_scale:
        ...
        c = cos(k[2])
        s = sin(k[2])
        return array([[c, -s], [s, c]]), array([k[0], k[1]])
    ...
    # Calculate affine transformation
    k = dot(inv(A), b)
    return array([[k[0] + 1, k[1]], [k[3], k[4] + 1]]), array([k[2], k[5]])


def apply_transform(img: Union[ndarray, ma.MaskedArray],
                    mat: Optional[ndarray],
                    offset: ndarray,
                    ref_width: int, ref_height: int,
    ...
    if mat is None:
        ...
        mask = scipy.ndimage.shift(
            mask, offset, cval=1, prefilter=prefilter) > 0.06
    else:
